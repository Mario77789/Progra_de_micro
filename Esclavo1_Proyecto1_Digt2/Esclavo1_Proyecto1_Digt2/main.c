...
static void twi_slave_init(uint8_t addr)
{
    TWAR = (addr << 1);                                  // Dirección del esclavo
    TWCR = (1<<TWEN) | (1<<TWEA) | (1<<TWIE) | (1<<TWINT);
    // TWEN: habilita TWI; TWEA: ACK automático; TWIE: interrupción; TWINT: limpia flag
}
...
ISR(TWI_vect)
{
    ...
    case TW_ST_SLA_ACK:
    case TW_ST_DATA_ACK:
        tx_byte = last_distance_cm; // sirve el último disponible (0xFF si aún no)
        TWDR = tx_byte;
        TWCR = (1<<TWEN)|(1<<TWEA)|(1<<TWIE)|(1<<TWINT);
        break;
